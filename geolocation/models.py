...
class Coordinates(models.Model):
    ...
    def to_address(self):
        ...
        request = requests.get(
            f"https://maps.googleapis.com/maps/api/geocode/json?latlng={self.to_csv()}&key={GOOGLE_KEY}")
        response = dict(request.json())
        db_logger.info(response)
        try:
            address_1 = response['results'][0]["address_components"][0]['long_name'] + " " + \
...
class Address(models.Model):
    ...
    def to_coordinates(self):
        ...
        try:
            request = requests.get(
                f"https://maps.googleapis.com/maps/api/geocode/json?address={self.__str__()}&key={GOOGLE_KEY}")
